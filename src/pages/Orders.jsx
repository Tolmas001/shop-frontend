...
const Orders = () => {
  ...
  const { t, formatPrice, backendUrl, handleImageError } = useApp();
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', borderBottom: '1px solid #f8f8f8', paddingBottom: '20px' }}>
              ...
              <span className={`status-badge ${getStatusClass(order.status)}`} style={{ 
                ...
                background: order.status === 'pending' ? '#FEF3C7' : (order.status === 'delivered' ? '#D1FAE5' : (order.status === 'cancelled' ? '#FEE2E2' : '#DBEAFE')),
                color: order.status === 'pending' ? '#D97706' : (order.status === 'delivered' ? '#059669' : (order.status === 'cancelled' ? '#DC2626' : '#2563EB'))
              }}>
              ...
              {order.items_list && order.items_list.map((item, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: '13px', color: '#86868b' }}>{item.quantity} dona x {formatPrice(item.price)}</p>
                  </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '20px', borderTop: '1px dotted #e5e7eb' }}>
              ...
              <span style={{ fontSize: '20px', fontWeight: 800, color: 'var(--primary)' }}>{formatPrice(order.total_amount)}</span>
            </div>
          </motion.div>
        ))}
      </div>
